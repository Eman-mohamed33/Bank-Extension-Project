...
void SHOWBalancesClients(ClientData CD)
{
    cout << "|" << left << setw(18) << CD.Account_Number << "| " << left << setw(48) << CD.Client_Name << " |" << left << setw(12) << CD.AccountBalance << endl;

}
void Total_Balances(string fileName)
{
    ...
    cout << "_____________________________________________________________________________________________________________________" << endl;
    cout << endl;
    cout << setw(20) << "|  Account Number  |" << setw(30) << "  Client Name                                     |" << setw(12) << " Balance  " << endl;
    cout << "_____________________________________________________________________________________________________________________" << endl;
    cout << endl;
    for (ClientData& CD : VClient)
    {
        SHOWBalancesClients(CD);
        TotalBalances = TotalBalances + CD.AccountBalance;
    }
    cout << "_____________________________________________________________________________________________________________________" << endl;
    cout << endl;
...
void DepositeToAccountBalance(string filename)
{
    vector <ClientData> VClient = PrintDataFromFile(filename);
    system("cls");
    system("color 0f");
    ...
    cout << "Please Enter AccountNumber ?  ";
    getline(cin >> ws, accnum);
    while (!SearchClientData(VClient, accnum, CD))
    {
        cout << endl;
        cout << "Client With Account Number (" << accnum << ") Does Not Exist." << endl;
        cout << endl;
        cout << "Please Enter AccountNumber ?  ";
        getline(cin >> ws, accnum);
    }
        ...
        if (yesOrno == 'y' || yesOrno == 'Y')
        {
            for (ClientData& CD : VClient)
            {
                if (CD.Account_Number == accnum)
                {

                    CD.AccountBalance = CD.AccountBalance + depositamount;
                    Pan = CD.AccountBalance;
                    break;
                }
            }
            saveVectorTofile(filename, VClient);
            VClient = PrintDataFromFile(filename);
            cout << endl;
            cout << "Done Successfully, New Balance = " << Pan << endl;
            cout << endl;
...
void WithDrawFromAccountBalance(string filename)
{
    vector <ClientData> VClient = PrintDataFromFile(filename);
    system("cls");
    system("color 0f");
    ...
    cout << "Please Enter AccountNumber ?  ";
    getline(cin >> ws, accnum);
    while (!ClientExisetByAccountNumber(accnum))
    {
        cout << endl;
        cout << "Client With Account Number (" << accnum << ") Does Not Exist." << endl;
        cout << endl;
    ...
    if (SearchClientData(VClient, accnum, CD))
    {
        cout << endl;
        ClientDetails(CD);
        cout << endl;
        ...
        cin >> withdrawamount;
        while (withdrawamount > CD.AccountBalance)
        {
            cout << endl;
            cout << "Amount Exceeds The Balance, You can Withdraw Up To : " << CD.AccountBalance << endl;
        ...
        if (yesOrno == 'y' || yesOrno == 'Y')
        {
            for (ClientData& CD : VClient)
            {
                if (CD.Account_Number == accnum)
                {
                    ...
                    CD.AccountBalance = CD.AccountBalance - withdrawamount;
                    Pan = CD.AccountBalance;
                    break;
                }
            }
            ...
            VClient = PrintDataFromFile(filename);
            cout << endl;
            cout << "Done Successfully, New Balance is " << Pan << endl;
            cout << endl;
...
void BackToShowTransactionsMenu()
{
    cout << "Press Any Key To Go Back To Transactions Menue...";
    system("pause>0");
    Show_Transactions();
}
void Transactions(short userchoice)
{
    switch (userchoice)
    {
    case 1:
        DepositeToAccountBalance(filename);
        BackToShowTransactionsMenu();
        break;
    case 2:
        WithDrawFromAccountBalance(filename);
        BackToShowTransactionsMenu();
        break;
    case 3:
        Total_Balances(filename);
        BackToShowTransactionsMenu();
        break;
    case 4:
        Main_Menu_Screen();
